...
static int print_location_send = 4;
static int print_location_recv = 5;
static char blank[] = {"                                                  "};
...
void *send(void *arg) {
    ...
    while(1) {
        ...
        myPrintf("[Thread B-send] Trying to send to mbox2...\n", print_location_send);
        sys_mbox_send(*mbox_p, send_buf, send_length);
        myPrintf("[Thread B-send] Sent to mbox2               \n", print_location_send);
...
void *recv(void *arg) {
    ...
    while(1) {
        ...
        myPrintf("[Thread B-recv] Trying to recv from mbox1...\n", print_location_recv);
        sys_mbox_recv(*mbox_p, recv_buf, recv_length);
        myPrintf("[Thread B-recv] Received from mbox1          \n", print_location_recv);
        int sleep_time = rand() % 3 + 1;
        sys_sleep(sleep_time);
    }
